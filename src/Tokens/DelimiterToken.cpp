...
DelimiterToken::DelimiterToken(DelimiterType delimiterType, std::wstring::const_iterator pos)
	: Token(Token::Delimiter, static_cast<int>(delimiterType), pos) {
...
std::unique_ptr<Token> DelimiterToken::getToken(std::wstring::const_iterator& start, std::wstring::const_iterator& end, Token* previousToken) {
	std::wsmatch match;
	if (std::regex_search(start, end, match, s_regex)) {
		if (start != match.prefix().second) {
			return nullptr;;
		}
		auto ptr = std::make_unique<DelimiterToken>(delimiterTypeStringMap[match[0]], start);
