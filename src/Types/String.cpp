...
std::shared_ptr<Value> String::Subtraction(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do subtraction operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do subtraction operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do subtraction operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do subtraction operation between type string and type string");
	}
...
std::shared_ptr<Value> String::Multiplication(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do multiplication operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do multiplication operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do multiplication operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do multiplication operation between type string and type string");
	}
...
std::shared_ptr<Value> String::Division(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do division operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do division operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do division operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do division operation between type string and type string");
	}
...
std::shared_ptr<Value> String::Modulus(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do modulo operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do modulo operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do modulo operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do modulo operation between type string and type string");
	}
...
std::shared_ptr<Value> String::Exponentiation(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do exponentiation operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do exponentiation operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do exponentiation operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do exponentiation operation between type string and type string");
	}
...
std::shared_ptr<Value> String::EqualTo(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do equal to operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do equal to operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do equal to operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		auto rvalue = rOperand->get<std::wstring>();
...
std::shared_ptr<Value> String::NotEqualTo(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do not equal to operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do not equal to operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do not equal to operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		auto rvalue = rOperand->get<std::wstring>();
		return makeValue<bool>(lOperand->get<std::wstring>() != rvalue);
...
std::shared_ptr<Value> String::Greater(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do greater operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do greater operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do greater operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		auto rvalue = rOperand->get<std::wstring>();
		return makeValue<bool>(lOperand->get<std::wstring>() > rvalue);
...
std::shared_ptr<Value> String::Less(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do less operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do less operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do less operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		auto rvalue = rOperand->get<std::wstring>();
		return makeValue<bool>(lOperand->get<std::wstring>() < rvalue);
...
std::shared_ptr<Value> String::GreaterEqual(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do greater equal operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do greater equal operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do greater equal operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		auto rvalue = rOperand->get<std::wstring>();
		return makeValue<bool>(lOperand->get<std::wstring>() >= rvalue);
...
std::shared_ptr<Value> String::LessEqual(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do less equal operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do less equal operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do less equal operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		auto rvalue = rOperand->get<std::wstring>();
		return makeValue<bool>(lOperand->get<std::wstring>() <= rvalue);
...
std::shared_ptr<Value> String::BitwiseAnd(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do bitwise and operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do bitwise and operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do bitwise and operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do bitwise and operation between type string and type string");
	}
...
std::shared_ptr<Value> String::BitwiseOr(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do bitwise or operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do bitwise or operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do bitwise or operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do bitwise or operation between type string and type string");
	}
...
std::shared_ptr<Value> String::BitwiseXor(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do bitwise xor operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do bitwise xor operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do bitwise xor operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do bitwise xor operation between type string and type string");
	}
...
std::shared_ptr<Value> String::LeftShift(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do leftshift operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do leftshift operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do leftshift operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do leftshift operation between type string and type string");
	}
...
std::shared_ptr<Value> String::RightShift(std::shared_ptr<Value> lOperand, std::shared_ptr<Value> rOperand) {
	if (rOperand->type == L"int") {
		throw std::runtime_error("Cannot do rightshift operation between type string and type int");
	}
	else if (rOperand->type == L"float") {
		throw std::runtime_error("Cannot do rightshift operation between type string and type float");
	}
	else if (rOperand->type == L"bool") {
		throw std::runtime_error("Cannot do rightshift operation between type string and type bool");
	}
	else if (rOperand->type == L"string") {
		throw std::runtime_error("Cannot do rightshift operation between type string and type string");
	}
...
std::shared_ptr<Value> String::UnaryNegation(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do negation operation on type float");
}

std::shared_ptr<Value> String::UnaryPlus(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do plus operation on type float");
}

std::shared_ptr<Value> String::UnaryAbs(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do absolute operation on type float");
}
...
std::shared_ptr<Value> String::BitwiseNot(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do bitwise not operation on type float");
}

std::shared_ptr<Value> String::FrontIncrement(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do front increment operation on type float");
}

std::shared_ptr<Value> String::FrontDecrement(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do front decrement operation on type float");
}

std::shared_ptr<Value> String::EndIncrement(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do end increment operation on type float");
}

std::shared_ptr<Value> String::EndDecrement(std::shared_ptr<Value> lOperand) {
	throw std::runtime_error("Cannot do end decrement operation on type float");
}
